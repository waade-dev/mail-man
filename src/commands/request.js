 ...
 * If the request already exists, loads its values as defaults (edit mode).
 */
...
// ─────────────────────────────────────────────────────────────
//  Interactive field prompts  (shared between add + edit)
// ─────────────────────────────────────────────────────────────
...
async function addRequest(pathStr) {
  ...
  // Check if this is a new or existing request
  const cols     = await store.getCollections();
  const colIsNew = !cols.includes(collection);
  const existing = colIsNew ? null : await store.getRequest(collection, request);
  ...
  await store.saveRequest(collection, reqObj);
