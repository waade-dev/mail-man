 ...
 *
 * Fetches request definition + active env from the server, builds an HTML
 * viewer, and opens it in Chrome.  The ephemeral HTTP server is local.
 */
...
function methodColor(m) {
  const map = { GET:'#6a9955',POST:'#dcdcaa',PUT:'#569cd6',PATCH:'#c586c0',DELETE:'#f44747',HEAD:'#9cdcfe',OPTIONS:'#858585' };
...
:root{--bg:#1e1e1e;--surface:#252526;--surface2:#2d2d30;--border:#3c3c3c;--text:#d4d4d4;--dim:#858585;--dim2:#6a6a6a;
  --blue:#569cd6;--cyan:#9cdcfe;--green:#6a9955;--yellow:#dcdcaa;--orange:#ce9178;--purple:#c586c0;--red:#f44747}
...
.method-badge{padding:4px 12px;border-radius:4px;font-size:13px;font-weight:700;color:#111;background:${mColor}}
...
.spacer{flex:1}.timer{color:var(--dim);font-size:12px}
#content{padding:20px;display:flex;flex-direction:column;gap:20px}
.desc{color:var(--dim);font-size:12px;font-style:italic;padding:8px 12px;background:var(--surface);border-radius:4px;border-left:3px solid var(--border)}
.section{background:var(--surface);border:1px solid var(--border);border-radius:6px;overflow:hidden}
.section-head{padding:8px 14px;display:flex;align-items:center;gap:8px;border-bottom:1px solid var(--border);cursor:pointer;user-select:none}
...
.section-body{padding:14px;display:none}.section-body.open{display:block}
.url-block{display:flex;flex-direction:column;gap:6px}
.url-raw{color:var(--dim2);font-size:12px}.url-resolved{color:var(--cyan);font-size:13px;word-break:break-all}
.url-label{font-size:11px;color:var(--dim);text-transform:uppercase;letter-spacing:.5px}
table{width:100%;border-collapse:collapse;font-size:12.5px}
td{padding:4px 8px;vertical-align:top}tr:not(:last-child) td{border-bottom:1px solid var(--border)}
...
.jk{color:var(--cyan)}.js{color:var(--orange)}.jn{color:var(--yellow)}.jb{color:var(--purple)}.jp{color:var(--dim2)}
...
<div id="content">
  ${descHtml}
  <div class="section">
    <div class="section-head" onclick="toggleSection(this)"><span class="section-arrow open">▸</span><span class="section-label">URL</span></div>
    <div class="section-body open">
      <div class="url-block">
        ${urlChanged ? `<div><div class="url-label">Raw (with variables)</div><div class="url-raw">${esc(req.url)}</div></div>
        <div><div class="url-label">Resolved (${esc(envName)})</div><div class="url-resolved">${esc(resolved.url)}</div></div>`
          : `<div class="url-resolved">${esc(req.url)}</div>`}
      </div>
    </div>
  </div>
  ${hasHeaders ? `<div class="section">
    <div class="section-head" onclick="toggleSection(this)"><span class="section-arrow open">▸</span><span class="section-label">Headers</span><span class="meta-pill">${Object.keys(req.headers).length}</span></div>
    <div class="section-body open"><table>${Object.entries(req.headers).map(([k,v])=>`<tr><td>${esc(k)}</td><td class="val-orange">${esc(v)}</td></tr>`).join('')}</table></div>
  </div>` : ''}
  ${hasAuth ? `<div class="section">
    <div class="section-head" onclick="toggleSection(this)"><span class="section-arrow open">▸</span><span class="section-label">Auth</span><span class="meta-pill">${esc(req.auth.type)}</span></div>
    <div class="section-body open"><table>${authRows}</table></div>
  </div>` : ''}
  ${hasBody ? `<div class="section">
    <div class="section-head" onclick="toggleSection(this)"><span class="section-arrow open">▸</span><span class="section-label">Body</span></div>
    <div class="section-body open"><pre class="json-pre" id="body-output"></pre></div>
  </div>` : ''}
  <div class="section">
    <div class="section-head" onclick="toggleSection(this)"><span class="section-arrow">▸</span><span class="section-label">cURL</span></div>
    <div class="section-body"><pre class="curl-pre">${esc(curlStr)}</pre></div>
  </div>
</div>
<script>
'use strict';
...
if(RAW_BODY!==null){document.getElementById('body-output').innerHTML=renderNode(RAW_BODY,0);}
function toggleSection(h){const a=h.querySelector('.section-arrow'),b=h.nextElementSibling;a.classList.toggle('open');b.classList.toggle('open');}
...
function copyCurl(){navigator.clipboard.writeText(CURL_STR).then(()=>{const b=document.getElementById('copy-curl');b.textContent='✓ Copied!';b.classList.add('copied');setTimeout(()=>{b.textContent='Copy as cURL';b.classList.remove('copied');},2000);});}
...
function renderNode(v,i){
  ...
  const k=Object.keys(v);if(!k.length)return '<span class="jp">{}</span>';
...
async function beautifyReq(pathStr) {
  ...
  // Fetch request + state in parallel
  ...
  info(`Request viewer → ${chalk.cyan.underline(viewUrl)}`);
